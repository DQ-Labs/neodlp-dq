        ...
        try {
            ...
            if (output.code === 0) {
                ...
                if (currentPlatform === 'windows') {
                    ...
                    // Runs in Rust so the webview needs no permission to spawn powershell
                    try {
                        await invoke('update_ytdlp_elevated', { channel: ytDlpUpdateChannel });
                        console.log("yt-dlp updated successfully with elevation");
                        LOG.info('NEODLP', "yt-dlp updated successfully with elevation");
                        saveKvPair('ytdlp_update_last_check', CURRENT_TIMESTAMP);
                        setYtDlpVersion(null);
                        toast.success("Update successful", { description: "yt-dlp has been updated successfully." });
                    } catch (elevateError) {
                        console.error("Failed to update yt-dlp with elevation:", elevateError);
                        LOG.error('NEODLP', `Failed to update yt-dlp with elevation: ${elevateError}`);
